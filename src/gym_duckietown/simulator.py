    ...
    def reset(self, segment: bool = False):
        ...
        if self.start_pose is not None:
            ...
            for _ in range(MAX_SPAWN_ATTEMPTS):
                ...
                x = 1#self.np_random.uniform(i, i + 1) * self.road_tile_size
                z = 3#self.np_random.uniform(j, j + 1) * self.road_tile_size
                propose_pos = np.array([x, 0, z])

                # Choose a random direction
                propose_angle = 90#self.np_random.uniform(0, 2 * math.pi)
